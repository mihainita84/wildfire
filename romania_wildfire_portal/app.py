...
.brand-strip {display:flex;gap:14px;align-items:center;justify-content:flex-start;flex-wrap:wrap;margin:2px 0 12px 0;}
.brand-strip img {height:68px;width:auto;max-width:47%;border-radius:12px;}
...
prepare_logo = (Path(__file__).parent / "assets" / "prepare_wp4_wordmark.svg").read_text(encoding="utf-8")
unitbv_logo = (Path(__file__).parent / "assets" / "unitbv_wordmark.svg").read_text(encoding="utf-8")
prepare_uri = "data:image/svg+xml;base64," + base64.b64encode(prepare_logo.encode("utf-8")).decode("ascii")
unitbv_uri = "data:image/svg+xml;base64," + base64.b64encode(unitbv_logo.encode("utf-8")).decode("ascii")
st.markdown(
    f'<div class="brand-strip"><img src="{prepare_uri}" alt="PREPARE WP4"><img src="{unitbv_uri}" alt="UNITBV"></div>',
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    st.caption(t("După analiză, portalul generează miniaturi separate pentru WorldCover, ESA CCI Biomass, harta de combustibil EFFIS, hotspot-uri GWIS MODIS și indicii EFFIS de pericol de incendiu.", "After analysis, the portal generates separate thumbnails for WorldCover, ESA CCI Biomass, the EFFIS fuel map, GWIS MODIS hotspots and EFFIS fire-danger indices."))
...
def cached_effis_gallery(geojson_str: str, day_text: str):
    aoi = geojson_to_gdf(json.loads(geojson_str))
    product_order = ["FUEL", "FWI", "FFMC", "DMC", "DC", "ISI", "BUI", "ANOMALY", "RANKING", "MODIS_HS"]
    products, errors = {}, {}
...
effis_rows = []
for short_name in ["FUEL", "FWI", "FFMC", "DMC", "DC", "ISI", "BUI", "ANOMALY", "RANKING", "MODIS_HS"]:
    effis_rows.append({
...
st.caption(t(
    "EFFIS utilizează sistemul Canadian Fire Weather Index pentru o evaluare armonizată în Europa. ECMWF furnizează prognoze la aproximativ 8 km; Anomaly și Ranking compară situația curentă cu o serie istorică de aproximativ 40 de ani.",
    "EFFIS uses the Canadian Fire Weather Index system for harmonized European assessment. ECMWF supplies forecasts at about 8 km; Anomaly and Ranking compare current conditions with an approximately 40-year historical series.",
))
...
if thumbs or effis_products:
    ...
    if effis_products:
        ...
        st.caption(t(
            "Aceste miniaturi sunt descărcate ca imagini WMS reale pe server, validate și apoi decupate într-un context regional de aproximativ 150–200 km în jurul AOI. Pentru straturile temporale portalul încearcă mai întâi intervalul TIME=ziua precedentă/ziua selectată, similar linkurilor publicate de EFFIS.",
            "These thumbnails are downloaded as real WMS images on the server, validated, then shown in an approximately 150–200 km regional context around the AOI. For time-dependent layers the portal first tries TIME=previous-day/selected-day, matching the pattern published by EFFIS.",
        ))
        ...
        effis_order = ["FWI", "FFMC", "DMC", "DC", "ISI", "BUI", "ANOMALY", "RANKING"]
        for i in range(0, len(effis_order), 4):
            ...
            for col, short_name in zip(cols, effis_order[i:i+4]):
                with col:
                    ...
                    meta = f"{product.get('time')} • ECMWF ~8 km • {product.get('layer')}"
                    st.markdown(
                        _thumb_card(
