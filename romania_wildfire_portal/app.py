...
with st.sidebar:
    ...
    show_biomass = st.checkbox(f"ESA CCI Biomass {CCI_YEAR} (100 m)", True)
    ...
    st.caption(t(
        "OpenStreetMap este singurul strat de bază. WorldCover, ESA CCI Biomass, EFFIS și FIRMS sunt plasate în panouri Leaflet dedicate, cu prioritate explicită deasupra hărții de bază.",
        "OpenStreetMap is the only basemap. WorldCover, ESA CCI Biomass, EFFIS and FIRMS use dedicated Leaflet panes with explicit priority above the basemap.",
    ))
    ...
    st.caption(t(
        "Stivă hartă: OpenStreetMap → WorldCover → ESA CCI Biomass → EFFIS FWI → NASA FIRMS. Pentru România, biomasa CCI este citită din dala N50E020.",
        "Map stack: OpenStreetMap → WorldCover → ESA CCI Biomass → EFFIS FWI → NASA FIRMS. For Romania, CCI biomass is read from tile N50E020.",
    ))
...
def build_map():
    # One basemap only. Dedicated Leaflet panes guarantee that thematic layers
    # cannot be hidden below OpenStreetMap, even after layer toggling/reruns.
    m = folium.Map(
        location=[45.9, 24.9],
        zoom_start=7,
        tiles=None,
    ...
    # Explicit map stack, low -> high. Leaflet's default tile pane is around 200.
    CustomPane("worldcover_pane", z_index=420).add_to(m)
    CustomPane("biomass_pane", z_index=440).add_to(m)
    CustomPane("fwi_pane", z_index=460).add_to(m)
    CustomPane("firms_pane", z_index=650).add_to(m)
    ...
    if show_biomass:
        try:
            ...
            st.sidebar.warning(t(
                f"Biomasa CCI nu poate fi afișată: {exc}",
                f"CCI Biomass display unavailable: {exc}",
            ))
