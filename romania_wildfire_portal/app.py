...
if run:
    ...
    with st.spinner(t(
        ...
        # EFFIS thumbnails are rendered browser-side from direct WMS URLs.
        # This avoids server-side WMS/proxy failures on Streamlit hosting.
        effis_urls = {
            short_name: effis_thumbnail_url(aoi, fwi_day.isoformat(), layer_name)
            for short_name, layer_name in EFFIS_FORECAST_LAYERS.items()
        }
        st.session_state["effis_thumbnail_urls"] = effis_urls
...
effis_rows = []
for short_name in ["FWI", "FFMC", "DMC", "DC", "ISI", "BUI", "ANOMALY", "RANKING"]:
    effis_rows.append({
...
effis_urls = st.session_state.get("effis_thumbnail_urls", {})
...
if thumbs or effis_urls:
    ...
    if effis_urls:
        ...
        st.caption(t(
            "Miniaturile EFFIS sunt încărcate direct de browser din serviciul WMS oficial, nu prin serverul Streamlit. Astfel evităm problema din versiunea anterioară, în care cererile server-side produceau numai cele trei miniaturi locale.",
            "EFFIS thumbnails are loaded directly by the browser from the official WMS, rather than through the Streamlit server. This avoids the previous version's server-side request problem that left only the three local thumbnails.",
        ))
        ...
        for i in range(0, len(effis_order), 4):
            ...
            for col, short_name in zip(cols, effis_order[i:i+4]):
                with col:
                    ...
                    meta = f"{fwi_day.isoformat()} • ECMWF ~8 km"
                    st.markdown(
                        _thumb_card(
                            effis_urls[short_name],
                            f"EFFIS {short_name}",
                            meta,
                            desc,
                            "Copernicus EFFIS",
