...
# ESA Climate Change Initiative Biomass v7.0. The latest annual map in this
# release is 2024, at 100 m grid spacing. CEDA exposes public GeoTIFF tiles.
CCI_VERSION = "7.0"
CCI_YEAR = 2024
...
# A temporary cache avoids repeatedly downloading the same ~10 degree CEDA tile.
# It also works on Windows and on hosted Streamlit deployments.
...
def _tile_origins(bounds: tuple[float, float, float, float]) -> Iterable[tuple[int, int]]:
    ...
    lon_start = _tile_origin(west)
    lat_start = _tile_origin(south)
    lon_end = _tile_origin(east - 1e-10)
    lat_end = _tile_origin(north - 1e-10)
    for lat0 in range(lat_start, lat_end + 1, 10):
        for lon0 in range(lon_start, lon_end + 1, 10):
            yield lat0, lon0
...
def _download_tile(lat0: int, lon0: int, uncertainty: bool = False) -> Path:
    ...
    headers = {"User-Agent": "PREPARE-Romania-Wildfire-Portal/3.0"}
    try:
...
def _valid_values(arr: np.ndarray, nodata: float | int | None) -> np.ndarray:
    ...
    # CCI AGB is Mg/ha. Keep zero as a valid landscape value and reject sentinels.
    return vals[(vals >= 0) & (vals < 5000)]


def biomass_stats(aoi: gpd.GeoDataFrame, aoi_area_ha: float | None = None) -> dict:
    """Download only the CEDA 10-degree tile(s) intersecting the AOI and summarize AGB.

    For Romania almost all analyses use only N40E020, so the first request is
    typically one ~25 MB AGB tile plus its uncertainty tile; later calls use cache.
    """
    aoi = aoi.to_crs(4326)
    ...
    for lat0, lon0 in _tile_origins(tuple(aoi.total_bounds)):
        tile_geom = box(lon0, lat0, lon0 + 10, lat0 + 10)
...
def _rgba_from_agb(data: np.ndarray, nodata: float | int | None, vmax: float = 300.0) -> np.ndarray:
    ...
    if nodata is not None:
        valid &= arr != nodata
    # Hide zero/non-woody pixels so the basemap remains legible.
    visible = valid & (arr > 0)
    scaled = np.clip(arr / vmax, 0.0, 1.0)

    # Four-stop yellow-to-green biomass ramp.
    ...
    alpha = np.where(visible, 185, 0).astype("uint8")
...
def biomass_overlay_png(
    ...
    """Create a lightweight transparent PNG overlay for the Romanian map.

    The source remains the online 100 m CEDA GeoTIFF; only a downsampled display
    image is sent to Leaflet. AOI statistics still use the native 100 m cells.
    """
    west, south, east, north = bounds
    lat0, lon0 = _tile_origin(south), _tile_origin(west)
    # Romania lies in N40E020. If bounds are changed across a 10-degree edge,
    # callers should request a narrower overlay or extend this function to mosaic.
    if _tile_origin(north - 1e-10) != lat0 or _tile_origin(east - 1e-10) != lon0:
