...
def _lon_tile_origin(value: float) -> int:
    """Western edge of the 10-degree CCI tile."""
    return math.floor(value / 10.0) * 10


def _lat_tile_north(value: float) -> int:
    """Northern edge used in ESA CCI Biomass GeoTIFF tile names.

    Example: Romania (roughly 43-49 N) is in tile N50E020, which covers
    40-50 N and 20-30 E. The previous implementation incorrectly requested
    N40E020 because it treated the latitude code as the southern edge.
    """
...
def _tile_origins(bounds: tuple[float, float, float, float]) -> Iterable[tuple[int, int]]:
    """Yield (north_edge, west_edge) CCI tile identifiers intersecting bounds."""
...
def biomass_stats(aoi: gpd.GeoDataFrame, aoi_area_ha: float | None = None) -> dict:
    ...
    For Romania almost all analyses use only N50E020, so the first request is
    ...
    for lat0, lon0 in _tile_origins(tuple(aoi.total_bounds)):
        # CCI latitude code is the northern edge: N50 covers 40-50 N.
...
def biomass_overlay_png(
    ...
    # Romania lies in N50E020 (40-50 N, 20-30 E).
